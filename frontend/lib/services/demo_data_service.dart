class DemoDataService {
  static List<Map<String, dynamic>> roadNetworkPayload() {
    return [
      {
        'id': 'nh-45',
        'name': 'NH 45',
        'type': 'NH',
        'route': 'Chennai - Guduvanchery - Chengalpattu - Villupuram - Trichy - Dindigul',
        'districts': ['Chennai', 'Chengalpattu', 'Villupuram', 'Tiruchirappalli', 'Dindigul'],
        'length_km': 368,
        'year': 1956,
        'contractor': 'Dilip Buildcon Ltd.',
        'budget_crore': 3312,
        'condition': 'Good',
        'issues': ['Urban Encroachments', 'Minor Cracks', 'Widening pending'],
        'summary': 'NH | Good | 368 km',
      },
      {
        'id': 'sh-49',
        'name': 'State Highway 49',
        'type': 'SH',
        'route': 'Chennai - Mahabalipuram Coastal Belt',
        'districts': ['Chennai', 'Chengalpattu'],
        'length_km': 65,
        'year': 1984,
        'contractor': 'Tamil Nadu Roads Board',
        'budget_crore': 820,
        'condition': 'Moderate',
        'issues': ['Surface wear', 'Monsoon patchwork', 'Drainage issues'],
        'summary': 'SH | Moderate | 65 km',
      },
      {
        'id': 'mdr-001',
        'name': 'Sardar Patel Road Connector',
        'type': 'MDR',
        'route': 'Adyar - Saidapet link road',
        'districts': ['Chennai'],
        'length_km': 12,
        'year': 1998,
        'contractor': 'Metro CivilWorks Pvt Ltd.',
        'budget_crore': 54,
        'condition': 'Poor',
        'issues': ['Potholes', 'Broken shoulders', 'Drain cover damage'],
        'summary': 'MDR | Poor | 12 km',
      },
      {
        'id': 'mdr-002',
        'name': 'OMR - Tidel Stretch',
        'type': 'MDR',
        'route': 'Perungudi - Taramani service road',
        'districts': ['Chennai'],
        'length_km': 9,
        'year': 2004,
        'contractor': 'South Corridor Infra LLP',
        'budget_crore': 71,
        'condition': 'Moderate',
        'issues': ['Lane markings faded', 'Small cracks'],
        'summary': 'MDR | Moderate | 9 km',
      },
      {
        'id': 'mrd-003',
        'name': 'Guindy Inner Ring',
        'type': 'MDR',
        'route': 'Guindy - Kotturpuram connector',
        'districts': ['Chennai'],
        'length_km': 7,
        'year': 2010,
        'contractor': 'Smart Urban Infra',
        'budget_crore': 41,
        'condition': 'Good',
        'issues': ['Occasional congestion'],
        'summary': 'MDR | Good | 7 km',
      },
      {
        'id': 'nh-44',
        'name': 'NH 44',
        'type': 'NH',
        'route': 'Krishnagiri - Salem - Namakkal - Karur - Dindigul - Madurai - Tirunelveli - Kanyakumari',
        'districts': ['Krishnagiri', 'Dharmapuri', 'Salem', 'Namakkal', 'Karur', 'Dindigul', 'Madurai', 'Tirunelveli', 'Tenkasi', 'Kanyakumari'],
        'length_km': 627,
        'year': 1960,
        'contractor': 'NHAI / L&T Construction Ltd.',
        'budget_crore': 5643,
        'condition': 'Moderate',
        'issues': ['Heavy Traffic Congestion', 'Potholes', 'Widening pending'],
        'summary': 'NH | Moderate | 627 km',
      },
      {
        'id': 'nh-48',
        'name': 'NH 48',
        'type': 'NH',
        'route': 'Chennai - Ranipet - Vellore - Krishnagiri',
        'districts': ['Chennai', 'Tiruvallur', 'Vellore', 'Ranipet', 'Krishnagiri'],
        'length_km': 210,
        'year': 1955,
        'contractor': 'GR Infraprojects Ltd.',
        'budget_crore': 2310,
        'condition': 'Good',
        'issues': ['Toll Plaza Queues', 'Faded Lane Markings'],
        'summary': 'NH | Good | 210 km',
      },
      {
        'id': 'nh-66',
        'name': 'NH 66',
        'type': 'NH',
        'route': 'Chennai - Villupuram - Cuddalore - Nagapattinam - Thanjavur - Ramanathapuram - Thoothukudi',
        'districts': ['Chennai', 'Villupuram', 'Cuddalore', 'Nagapattinam', 'Thanjavur', 'Pudukkottai', 'Ramanathapuram', 'Thoothukudi'],
        'length_km': 490,
        'year': 1962,
        'contractor': 'NCC Ltd.',
        'budget_crore': 4410,
        'condition': 'Moderate',
        'issues': ['Coastal Flood Damage', 'Potholes', 'Shoulder Erosion'],
        'summary': 'NH | Moderate | 490 km',
      },
      {
        'id': 'nh-67',
        'name': 'NH 67',
        'type': 'NH',
        'route': 'Nagapattinam - Karur - Coimbatore - Gudalur - Mysore corridor',
        'districts': ['Nagapattinam', 'Tiruvarur', 'Thanjavur', 'Tiruchirappalli', 'Karur', 'Namakkal', 'Erode', 'Coimbatore', 'Nilgiris'],
        'length_km': 379,
        'year': 1960,
        'contractor': 'Dilip Buildcon Ltd.',
        'budget_crore': 3411,
        'condition': 'Moderate',
        'issues': ['Overloaded Trucks', 'Ghat Sections', 'Potholes'],
        'summary': 'NH | Moderate | 379 km',
      },
      {
        'id': 'nh-85',
        'name': 'NH 85',
        'type': 'NH',
        'route': 'Hosur - Krishnagiri - Dharmapuri - Salem',
        'districts': ['Krishnagiri', 'Dharmapuri', 'Salem'],
        'length_km': 145,
        'year': 1980,
        'contractor': 'GVR Infra Projects Ltd.',
        'budget_crore': 1305,
        'condition': 'Moderate',
        'issues': ['Heavy Granite Trucks', 'Potholes', 'Missing Signage'],
        'summary': 'NH | Moderate | 145 km',
      },
      {
        'id': 'nh-136',
        'name': 'NH 136',
        'type': 'NH',
        'route': 'Walajapet - Kanchipuram - Chengalpattu',
        'districts': ['Ranipet', 'Kanchipuram', 'Chengalpattu'],
        'length_km': 98,
        'year': 1975,
        'contractor': 'PNC Infratech Ltd.',
        'budget_crore': 980,
        'condition': 'Good',
        'issues': ['Urban Traffic', 'Pedestrian Issues', 'Temple Route Congestion'],
        'summary': 'NH | Good | 98 km',
      },
      {
        'id': 'nh-205',
        'name': 'NH 205',
        'type': 'NH',
        'route': 'Chennai - Tiruvallur - Andhra border',
        'districts': ['Chennai', 'Tiruvallur'],
        'length_km': 68,
        'year': 1960,
        'contractor': 'L&T Construction Ltd.',
        'budget_crore': 748,
        'condition': 'Good',
        'issues': ['Heavy Industrial Traffic', 'Outer Ring Road Merge Pending'],
        'summary': 'NH | Good | 68 km',
      },
      {
        'id': 'nh-208',
        'name': 'NH 208',
        'type': 'NH',
        'route': 'Tenkasi - Shencottah - Punalur corridor',
        'districts': ['Tenkasi', 'Tirunelveli'],
        'length_km': 92,
        'year': 1975,
        'contractor': 'Southern Structurals Ltd.',
        'budget_crore': 828,
        'condition': 'Poor',
        'issues': ['Flooding', 'ROB Work Pending', 'Potholes'],
        'summary': 'NH | Poor | 92 km',
      },
      {
        'id': 'nh-336',
        'name': 'NH 336',
        'type': 'NH',
        'route': 'Tiruvannamalai - Gingee - Villupuram',
        'districts': ['Tiruvannamalai', 'Villupuram'],
        'length_km': 92,
        'year': 1988,
        'contractor': 'Arss Infrastructure Projects Ltd.',
        'budget_crore': 828,
        'condition': 'Good',
        'issues': ['Temple Traffic Peak Season', 'Speed Humps'],
        'summary': 'NH | Good | 92 km',
      },
      {
        'id': 'nh-532',
        'name': 'NH 532',
        'type': 'NH',
        'route': 'Cuddalore - Vridhachalam - Salem',
        'districts': ['Cuddalore', 'Villupuram', 'Kallakurichi', 'Salem'],
        'length_km': 192,
        'year': 2019,
        'contractor': 'NHAI / GR Infraprojects Ltd.',
        'budget_crore': 1920,
        'condition': 'Good',
        'issues': ['Widening Underway', 'Roadside Stalls'],
        'summary': 'NH | Good | 192 km',
      },
    ];
  }

  static Map<String, dynamic> roadPayload() {
    return {
      'overview': {
        'overall_score': 58,
        'total_roads': 6,
        'red_roads': 2,
        'yellow_roads': 2,
        'green_roads': 2,
      },
      'roads': [
        {
          'id': 'road-001',
          'name': 'Sardar Patel Road',
          'ward': 'Adyar',
          'polyline': [
            {'lat': 12.9928, 'lng': 80.2297},
            {'lat': 12.9941, 'lng': 80.2364},
            {'lat': 12.9968, 'lng': 80.2438}
          ],
          'road_health_score': 38,
          'color': 'red',
          'nearby_issues': 14,
          'recent_complaints': 9
        },
        {
          'id': 'road-002',
          'name': 'OMR - Tidel Stretch',
          'ward': 'Perungudi',
          'polyline': [
            {'lat': 12.9764, 'lng': 80.2496},
            {'lat': 12.9708, 'lng': 80.2522},
            {'lat': 12.9651, 'lng': 80.2549}
          ],
          'road_health_score': 61,
          'color': 'yellow',
          'nearby_issues': 7,
          'recent_complaints': 5
        },
        {
          'id': 'road-004',
          'name': 'GST Road - Guindy Junction',
          'ward': 'Guindy',
          'polyline': [
            {'lat': 13.0085, 'lng': 80.2042},
            {'lat': 13.0032, 'lng': 80.2021},
            {'lat': 12.9987, 'lng': 80.1998}
          ],
          'road_health_score': 46,
          'color': 'red',
          'nearby_issues': 12,
          'recent_complaints': 11
        },
        {
          'id': 'road-005',
          'name': 'ECR Link Road',
          'ward': 'Thiruvanmiyur',
          'polyline': [
            {'lat': 12.9846, 'lng': 80.2622},
            {'lat': 12.9799, 'lng': 80.2668},
            {'lat': 12.9754, 'lng': 80.2717}
          ],
          'road_health_score': 74,
          'color': 'green',
          'nearby_issues': 4,
          'recent_complaints': 2
        }
      ],
      'recent_complaints': [
        {
          'id': 'cmp-2026-0001',
          'road_id': 'road-001',
          'description': 'Multiple deep potholes near bus stop causing bike skids.',
          'image_ref': 'demo_pothole_1.jpg',
          'location': {'lat': 12.9946, 'lng': 80.2371},
          'timestamp': '2026-04-28T08:41:00Z',
          'status': 'In Progress',
          'authority_ticket': 'CCMC-11408',
          'recommended_department': 'Highways Department',
          'routing_reason': 'Pothole repair and road surfacing issues are handled by highways.',
          'complaint_letter': 'Please inspect and repair the potholes near the bus stop on Sardar Patel Road.',
          'sent_to_authority': true,
          'delivered_to_authority': true,
          'read_by_authority': false,
          'sent_at': '2026-04-28T09:05:00Z',
          'delivered_at': '2026-04-28T09:15:00Z',
          'read_at': null,
          'timeline': [
            {
              'status': 'Filed',
              'at': '2026-04-28T08:41:00Z',
              'note': 'Complaint auto-filed from RoadWatch AI'
            },
            {
              'status': 'Sent',
              'at': '2026-04-28T09:05:00Z',
              'note': 'Complaint sent to Highways Department'
            },
            {
              'status': 'Delivered',
              'at': '2026-04-28T09:15:00Z',
              'note': 'Complaint letter delivered to the department inbox'
            }
          ]
        }
      ],
      'intelligence': {
        'damage_trends': [
          {'month': 'Jan', 'issues': 42},
          {'month': 'Feb', 'issues': 57},
          {'month': 'Mar', 'issues': 61},
          {'month': 'Apr', 'issues': 74},
          {'month': 'May', 'issues': 69}
        ],
        'budget_vs_condition': [
          {'road_id': 'road-001', 'allocated_inr': 5000000, 'score': 38},
          {'road_id': 'road-004', 'allocated_inr': 6300000, 'score': 46}
        ],
        'repair_frequency': [
          {'road_id': 'road-001', 'repairs_last_12m': 2},
          {'road_id': 'road-004', 'repairs_last_12m': 3}
        ],
        'prediction_text':
            'Sardar Patel Road may deteriorate further in 19 days if monsoon traffic remains high.'
      }
    };
  }

  static List<Map<String, dynamic>> budgetPayload() {
    return [
      {
        'road_id': 'road-001',
        'project_id': 'CHN-2025-ADY-11',
        'allocated_inr': 5000000,
        'spent_inr': 4700000,
        'contractor': 'Metro CivilWorks Pvt Ltd',
        'last_repair_date': '2025-12-03',
        'expected_score': 78,
        'actual_score': 38,
        'transparency_note':
            'Transparency alert: INR 5,000,000 allocated but score is 38/100'
      },
      {
        'road_id': 'road-002',
        'project_id': 'CHN-2025-PRG-07',
        'allocated_inr': 3500000,
        'spent_inr': 3260000,
        'contractor': 'South Corridor Infra LLP',
        'last_repair_date': '2025-11-18',
        'expected_score': 72,
        'actual_score': 61,
        'transparency_note':
            'Moderate mismatch between expected and AI-observed condition'
      }
    ];
  }

  static List<Map<String, dynamic>> contractorPayload() {
    return [
      {
        'id': 'con-001',
        'name': 'Metro CivilWorks Pvt Ltd',
        'company': 'Metro Construction',
        'project_status': 'Active',
        'overall_rating': 3.8,
        'total_reviews': 24,
        'trusted_badge': 'silver',
        'public_transparency_score': 72.5,
        'complaint_count': 3,
        'roads_managed': ['road-001', 'nh-45'],
        'profile_image_url': '',
        'reviews': [
          {
            'id': 'rev-001',
            'user_id': 'user-001',
            'user_name': 'Arjun K',
            'rating': 5,
            'sentiment': 'positive',
            'review_text': 'Excellent work on Sardar Patel Road. Great communication with residents.',
            'emotion_emoji': '😊',
            'timestamp': '2026-05-10T14:30:00Z',
            'is_spam_detected': false,
            'helpful_count': 12,
          },
          {
            'id': 'rev-002',
            'user_id': 'user-002',
            'user_name': 'Priya M',
            'rating': 3,
            'sentiment': 'neutral',
            'review_text': 'Work quality is good but project timeline exceeded expectations.',
            'emotion_emoji': '😐',
            'timestamp': '2026-05-08T10:15:00Z',
            'is_spam_detected': false,
            'helpful_count': 8,
          },
          {
            'id': 'rev-003',
            'user_id': 'user-003',
            'user_name': 'Rajesh V',
            'rating': 2,
            'sentiment': 'negative',
            'review_text': 'Poor quality control. Multiple defects found in final inspection.',
            'emotion_emoji': '😠',
            'timestamp': '2026-05-05T09:00:00Z',
            'is_spam_detected': false,
            'helpful_count': 5,
          },
        ],
      },
      {
        'id': 'con-002',
        'name': 'South Corridor Infra LLP',
        'company': 'Corridor Infrastructure',
        'project_status': 'Completed',
        'overall_rating': 4.2,
        'total_reviews': 18,
        'trusted_badge': 'gold',
        'public_transparency_score': 85.0,
        'complaint_count': 1,
        'roads_managed': ['road-002', 'sh-49'],
        'profile_image_url': '',
        'reviews': [
          {
            'id': 'rev-004',
            'user_id': 'user-004',
            'user_name': 'Anita S',
            'rating': 5,
            'sentiment': 'positive',
            'review_text': 'Outstanding project execution. Ahead of schedule and under budget.',
            'emotion_emoji': '😊',
            'timestamp': '2026-05-09T16:45:00Z',
            'is_spam_detected': false,
            'helpful_count': 15,
          },
          {
            'id': 'rev-005',
            'user_id': 'user-005',
            'user_name': 'Vikram D',
            'rating': 4,
            'sentiment': 'positive',
            'review_text': 'Professional team. Very responsive to complaints and suggestions.',
            'emotion_emoji': '😊',
            'timestamp': '2026-05-07T11:20:00Z',
            'is_spam_detected': false,
            'helpful_count': 10,
          },
        ],
      },
      {
        'id': 'con-003',
        'name': 'Dilip Buildcon Ltd',
        'company': 'Buildcon Group',
        'project_status': 'Active',
        'overall_rating': 3.5,
        'total_reviews': 21,
        'trusted_badge': 'bronze',
        'public_transparency_score': 68.0,
        'complaint_count': 5,
        'roads_managed': ['nh-45', 'nh-67'],
        'profile_image_url': '',
        'reviews': [
          {
            'id': 'rev-006',
            'user_id': 'user-006',
            'user_name': 'Suresh K',
            'rating': 4,
            'sentiment': 'positive',
            'review_text': 'Good work overall. Minor issues resolved quickly.',
            'emotion_emoji': '😊',
            'timestamp': '2026-05-06T13:10:00Z',
            'is_spam_detected': false,
            'helpful_count': 7,
          },
          {
            'id': 'rev-007',
            'user_id': 'user-007',
            'user_name': 'Divya R',
            'rating': 2,
            'sentiment': 'negative',
            'review_text': 'Frequent delays. Communication could be better.',
            'emotion_emoji': '😠',
            'timestamp': '2026-05-04T08:30:00Z',
            'is_spam_detected': false,
            'helpful_count': 4,
          },
        ],
      },
      {
        'id': 'con-004',
        'name': 'GR Infraprojects Ltd',
        'company': 'GR Infrastructure',
        'project_status': 'Active',
        'overall_rating': 4.5,
        'total_reviews': 32,
        'trusted_badge': 'gold',
        'public_transparency_score': 88.5,
        'complaint_count': 0,
        'roads_managed': ['nh-48'],
        'profile_image_url': '',
        'reviews': [
          {
            'id': 'rev-008',
            'user_id': 'user-008',
            'user_name': 'Meera N',
            'rating': 5,
            'sentiment': 'positive',
            'review_text': 'Best contractor we have worked with. Highly professional.',
            'emotion_emoji': '😊',
            'timestamp': '2026-05-09T14:00:00Z',
            'is_spam_detected': false,
            'helpful_count': 18,
          },
        ],
      },
      {
        'id': 'con-005',
        'name': 'NCC Ltd',
        'company': 'National Construction',
        'project_status': 'Completed',
        'overall_rating': 3.9,
        'total_reviews': 15,
        'trusted_badge': 'silver',
        'public_transparency_score': 75.0,
        'complaint_count': 2,
        'roads_managed': ['nh-66'],
        'profile_image_url': '',
        'reviews': [
          {
            'id': 'rev-009',
            'user_id': 'user-009',
            'user_name': 'Arun T',
            'rating': 4,
            'sentiment': 'positive',
            'review_text': 'Solid execution. Weather delays were unavoidable.',
            'emotion_emoji': '😊',
            'timestamp': '2026-05-08T15:25:00Z',
            'is_spam_detected': false,
            'helpful_count': 9,
          },
        ],
      },
    ];
  }
}
